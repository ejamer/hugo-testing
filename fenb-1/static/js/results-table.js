(function () {
  ...
  function enhanceTable(table, placeColIndex, hidePlacements) {
    table.classList.add('fenb-results-table');

    var tbody = table.querySelector('tbody');

    if (tbody && hidePlacements) {
    ...
    var sortState = Array(headers.length).fill(null);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var hidePlacements =
      body.dataset.hidePlacements === 'true';
    ...
    body.querySelectorAll('table').forEach(function (table) {
      ...
      enhanceTable(table, placeColIndex, hidePlacements);
    });
  });
})();